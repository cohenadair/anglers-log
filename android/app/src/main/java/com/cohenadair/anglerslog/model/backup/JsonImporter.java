...
public class JsonImporter {
    ...
    public static Date parseDate(String jsonString) {
        // fall back on an inaccurate importing date
        Date date = new Date();

        try {
            date = new SimpleDateFormat(Json.DATE_FORMAT_MS, Locale.US).parse(jsonString);
        } catch (ParseException e) {
            e.printStackTrace();

            // if failed, parse the old, less accurate format
            try {
                date = new SimpleDateFormat(Json.DATE_FORMAT, Locale.US).parse(jsonString);
                // This is not a great solution, but it fixes a very, very rare corner case that
                // happens if two catches have the same date string that was generated by
                // JsonExporter.dateToString, which originally was only accurate to the minute. It
                // has since been updated and is accurate to the millisecond.
                date.setTime(date.getTime() +
                        (new Random().nextInt(60000) + 1) -
                        (new Random().nextInt(60000) + 1));
