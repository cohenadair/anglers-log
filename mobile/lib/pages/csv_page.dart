...
class _CsvPageState extends State<CsvPage> {
  ...
  Future<XFile?> _exportCatches() async {
    ...
    var catchCustomEntityIds = UserPreferenceManager.get.catchFieldIds.where(
      (e) => _customEntityManager.entityExists(e),
    );
    var atmosphereFields =
    ...
    var fishingSpotIndex = catchFields.indexWhere(
      (e) => e.id == catchFieldIdFishingSpot,
    );
  ...
  Future<XFile?> _exportTrips() async {
    ...
    var tripCustomEntityIds = UserPreferenceManager.get.tripFieldIds.where(
      (e) => _customEntityManager.entityExists(e),
    );
    var atmosphereFields =
        UserPreferenceManager.get.tripFieldIds.isEmpty ||
            UserPreferenceManager.get.tripFieldIds.contains(
    ...
    for (var trip in trips) {
      ...
      for (var field in tripFields) {
        if (field.id == tripFieldIdTimeZone) {
          ...
          row.add(
            trip.hasWaterDepth() ? trip.waterDepth.displayValue(context) : "",
          );
        } else if (field.id == tripFieldIdWaterTemperature) {
          row.add(
            trip.hasWaterTemperature()
  ...
  void _addCustomEntityValuesToRow(
    ...
    for (var id in ids) {
      var value = customEntityValues.firstWhereOrNull(
        (e) => e.customEntityId == id,
      );
