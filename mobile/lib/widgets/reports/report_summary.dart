...
class _ReportSummaryState extends State<ReportSummary> {
  ...
  bool get _comparing => _models.length > 1;
  ...
  Widget _buildCatchesPerSpecies() => ExpandableChart<Species>(
        ...
        series: _models
            .map((model) => Series<Species>(
                model.catchesPerSpecies, model.displayDateRange))
            .toList(),
  ...
  Widget _buildSinceLastCatch() {
    if (!_hasCatches || _comparing || !_models.first.containsNow) {
      return Empty();
    }
  ...
  Set<String> _filters({
    ...
    bool includeDateRange = true,
      ...
      _models.fold<Set<String>>(
          {},
          (previousValue, model) => previousValue
            ..addAll(model.filters(
