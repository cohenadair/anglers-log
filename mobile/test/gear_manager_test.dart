...
void main() {
  ...
  testWidgets("Filtering by search: no matches", (tester) async {
    var id = randomId();
    await gearManager.addOrUpdate(Gear(id: id, name: "Ugly Stick Bass"));
    expect(
  ...
  testWidgets("Filtering by search: rod make and model", (tester) async {
    var id = randomId();
    await gearManager.addOrUpdate(Gear(id: id, rodMakeModel: "Ugly Stick"));
    expect(
      gearManager.matchesFilter(id, await buildContext(tester), "stick"),
      isTrue,
    );
  ...
  testWidgets("Filtering by search: rod serial number", (tester) async {
    var id = randomId();
    await gearManager.addOrUpdate(Gear(id: id, rodSerialNumber: "ABC123"));
    expect(
  ...
  testWidgets("Filtering by search: reel make and model", (tester) async {
    var id = randomId();
    await gearManager.addOrUpdate(Gear(id: id, reelMakeModel: "Pflueger"));
    expect(
  ...
  testWidgets("Filtering by search: reel serial number", (tester) async {
    var id = randomId();
    await gearManager.addOrUpdate(Gear(id: id, reelSerialNumber: "ABC123"));
    expect(
      gearManager.matchesFilter(id, await buildContext(tester), "ABC"),
      isTrue,
    );
  ...
  testWidgets("Filtering by search: reel size", (tester) async {
    var id = randomId();
    await gearManager.addOrUpdate(Gear(id: id, reelSize: "3000"));
    expect(
  ...
  testWidgets("Filtering by search: line make and model", (tester) async {
    var id = randomId();
    await gearManager.addOrUpdate(
      Gear(id: id, lineMakeModel: "FireLine Crystal"),
    );
    expect(
  ...
  testWidgets("Filtering by search: line color", (tester) async {
    var id = randomId();
    await gearManager.addOrUpdate(Gear(id: id, lineColor: "White"));
    expect(
  ...
  testWidgets("Filtering by search: hook make and model", (tester) async {
    var id = randomId();
    await gearManager.addOrUpdate(Gear(id: id, hookMakeModel: "Mustad Demon"));
    expect(
  ...
  testWidgets("Filtering by search: rod action", (tester) async {
    var id = randomId();
    var context = await buildContext(tester);
    await gearManager.addOrUpdate(Gear(id: id, rodAction: RodAction.fast));
  ...
  testWidgets("Filtering by search: rod power", (tester) async {
    var id = randomId();
    var context = await buildContext(tester);
    await gearManager.addOrUpdate(Gear(id: id, rodPower: RodPower.heavy));
    ...
    await gearManager.addOrUpdate(
      Gear(
        ...
        leaderRating: MultiMeasurement(
          mainValue: Measurement(unit: Unit.x, value: 3),
        ),
      ),
    );
    ...
    await gearManager.addOrUpdate(
      Gear(
        ...
        tippetLength: MultiMeasurement(
          mainValue: Measurement(unit: Unit.inches, value: 36),
        ),
      ),
    );
    ...
    await gearManager.addOrUpdate(
      Gear(
        ...
        tippetRating: MultiMeasurement(
          mainValue: Measurement(unit: Unit.x, value: 5),
        ),
      ),
    );
    ...
    await gearManager.addOrUpdate(
      Gear(
        ...
        hookSize: MultiMeasurement(
          mainValue: Measurement(unit: Unit.aught, value: 5),
        ),
      ),
    );
  ...
  testWidgets("numberOfCatches", (tester) async {
    ...
    when(managers.catchManager.list()).thenReturn([
      Catch(id: randomId(), gearIds: [gearId]),
      Catch(id: randomId()),
    ]);
  ...
  testWidgets("numberOfCatchQuantities", (tester) async {
    var gearId = randomId();
    when(managers.catchManager.list()).thenReturn([
      Catch(id: randomId(), gearIds: [gearId], quantity: 5),
      Catch(id: randomId()),
    ]);
