...
main() {
  testWidgets("Editing a name renders old name", (WidgetTester tester) async {
    await tester.pumpWidget(Testable((_) => SaveNamePage(
      title: Text("Title"),
      oldName: "Old name",
    )));
  ...
  testWidgets("Non-null onSave pops page if returns true", (WidgetTester tester)
      async
  {
    var observer = MockNavigatorObserver();
    bool popped = false;
  ...
  testWidgets("Invalid input disables save button", (WidgetTester tester)
      async
  {
    await tester.pumpWidget(Testable((_) => SaveNamePage(
      title: Text("Title"),
