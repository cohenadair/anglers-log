...
void main() {
  test("Default sortedIntMap", () {
    expect(
      ...
      {"6": 5678, "2": 1035, "3": 1000, "0": 200, "8": 109, "1": 100}.values,
    );
  });

  test("sortedIntMap with comparator", () {
    var map = {"0": 200, "6": 5678, "3": 1000, "2": 1035, "8": 109, "1": 100};

    expect(
      sortedIntMap<String>(
        map,
        (lhs, rhs) => map[lhs]!.compareTo(map[rhs]!),
      ).values,
      {"1": 100, "8": 109, "0": 200, "3": 1000, "2": 1035, "6": 5678}.values,
    );
  });
  ...
  test("valueOf", () {
    ...
    expect(
      valueOf<TestEnum>(TestEnum.values, TestEnum.values.length + 1),
      isNull,
    );
  });
