...
void main() {
  ...
  testWidgets("Null atmosphere shows label only", (tester) async {
    ...
    await tester.pumpWidget(
      Testable(
        (_) => AtmosphereInput(fetcher: fetcher, controller: controller),
      ),
    );
  ...
  testWidgets("Only fields in preferences are shown", (tester) async {
    ...
    ).thenReturn([atmosphereFieldIdTemperature, atmosphereFieldIdSkyCondition]);
  ...
  testWidgets("Editing atmosphere data", (tester) async {
    ...
    var newAtmosphere = Atmosphere(
      ...
      windSpeed: MultiMeasurement(
        system: MeasurementSystem.metric,
        mainValue: Measurement(unit: Unit.kilometers_per_hour, value: 8.5),
      ),
      windDirection: Direction.north_east,
      pressure: MultiMeasurement(
        system: MeasurementSystem.metric,
        mainValue: Measurement(unit: Unit.millibars, value: 1200),
      ),
      humidity: MultiMeasurement(
        system: MeasurementSystem.imperial_whole,
        mainValue: Measurement(unit: Unit.percent, value: 60),
      ),
      visibility: MultiMeasurement(
        system: MeasurementSystem.metric,
        mainValue: Measurement(unit: Unit.kilometers, value: 12),
      ),
  ...
  testWidgets("Updating units updates widgets", (tester) async {
    ...
    await UserPreferenceManager.get.setAirPressureSystem(
      MeasurementSystem.metric,
    );
    await UserPreferenceManager.get.setAirVisibilitySystem(
      MeasurementSystem.metric,
    );
    await UserPreferenceManager.get.setWindSpeedSystem(
      MeasurementSystem.metric,
    );
