...
void main() {
  ...
  group("ActionButton", () {
    testWidgets("Constructors", (tester) async {
      ...
      await tester.pumpWidget(
        Testable((context) => ActionButton.save(context)),
      );
      expect(find.text("SAVE"), findsOneWidget);

      await tester.pumpWidget(
        Testable((context) => ActionButton.cancel(context)),
      );
      expect(find.text("CANCEL"), findsOneWidget);

      await tester.pumpWidget(
        Testable((context) => ActionButton.edit(context)),
      );
      expect(find.text("EDIT"), findsOneWidget);

      await tester.pumpWidget(Testable((_) => ActionButton(text: "Test")));
    ...
    testWidgets("Text color", (tester) async {
      await tester.pumpWidget(
        Testable(
          (context) => ActionButton.done(context, textColor: Colors.red),
        ),
      );
      expect(
        (tester.firstWidget(find.text("DONE")) as Text).style!.color,
        Colors.red,
      );
    });
    ...
    testWidgets("Enabled", (tester) async {
      await tester.pumpWidget(
        Testable((context) => ActionButton.done(context, onPressed: () => {})),
      );
    ...
    testWidgets("Condensed", (tester) async {
      await tester.pumpWidget(
        Testable((context) => ActionButton.done(context, condensed: true)),
      );
  ...
  group("MinimumIconButton", () {
    ...
    testWidgets("Disabled color when disabled", (tester) async {
      await tester.pumpWidget(
        Testable(
          (_) => const MinimumIconButton(icon: Icons.group, color: Colors.red),
        ),
      );
  ...
  group("FloatingButton", () {
    testWidgets("Not pushed color", (tester) async {
      await tester.pumpWidget(
        Testable(
          (_) => FloatingButton.icon(icon: Icons.backpack, onPressed: () {}),
        ),
      );
    ...
    testWidgets("Android back button", (tester) async {
      ...
      expect(
        findFirstWithIcon<RawMaterialButton>(
          tester,
          Icons.arrow_back,
        ).onPressed,
        isNotNull,
    ...
    testWidgets("Close button", (tester) async {
      await tester.pumpWidget(Testable((_) => const FloatingButton.close()));
    ...
    testWidgets("Transparent background", (tester) async {
      await tester.pumpWidget(
        Testable(
          (_) =>
              FloatingButton(icon: Icons.delete, transparentBackground: true),
        ),
      );
    ...
    testWidgets("Custom size", (tester) async {
      await tester.pumpWidget(
        Testable((_) => const FloatingButton.smallIcon(icon: Icons.delete)),
      );
