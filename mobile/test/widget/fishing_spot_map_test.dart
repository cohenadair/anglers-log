...
void main() {
  ...
  testWidgets("Hybrid composition ignored for iOS", (tester) async {
    when(appManager.ioWrapper.isAndroid).thenReturn(false);

    await pumpMap(tester, FishingSpotMap());
    await mapController.finishLoading(tester);

    verifyNever(appManager.deviceInfoWrapper.androidInfo);
  });

  testWidgets("Hybrid composition true for Android 10+", (tester) async {
    when(appManager.ioWrapper.isAndroid).thenReturn(true);

    var buildVersion = MockAndroidBuildVersion();
    when(buildVersion.sdkInt).thenReturn(29);
    var deviceInfo = MockAndroidDeviceInfo();
    when(deviceInfo.version).thenReturn(buildVersion);
    when(appManager.deviceInfoWrapper.androidInfo)
        .thenAnswer((_) => Future.value(deviceInfo));

    await pumpMap(tester, FishingSpotMap());
    await mapController.finishLoading(tester);

    verify(appManager.deviceInfoWrapper.androidInfo).called(1);
    expect(MapboxMap.useHybridComposition, isTrue);
  });

  testWidgets("Hybrid composition false for null SDK value", (tester) async {
    when(appManager.ioWrapper.isAndroid).thenReturn(true);

    var buildVersion = MockAndroidBuildVersion();
    when(buildVersion.sdkInt).thenReturn(null);
    var deviceInfo = MockAndroidDeviceInfo();
    when(deviceInfo.version).thenReturn(buildVersion);
    when(appManager.deviceInfoWrapper.androidInfo)
        .thenAnswer((_) => Future.value(deviceInfo));
    ...
    verify(appManager.deviceInfoWrapper.androidInfo).called(1);
    expect(MapboxMap.useHybridComposition, isFalse);
  });

  testWidgets("Hybrid composition false for Android 9", (tester) async {
    when(appManager.ioWrapper.isAndroid).thenReturn(true);

    var buildVersion = MockAndroidBuildVersion();
    when(buildVersion.sdkInt).thenReturn(28);
    var deviceInfo = MockAndroidDeviceInfo();
    when(deviceInfo.version).thenReturn(buildVersion);
    when(appManager.deviceInfoWrapper.androidInfo)
        .thenAnswer((_) => Future.value(deviceInfo));
